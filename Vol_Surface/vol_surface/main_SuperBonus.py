        ...
        if model == "H":
            ...
            for i_stk, stk in enumerate(k_vol_surf):
                for j_mat, maturity in enumerate(T_vol_surf_y):
                    ...
                    mcCall[mod][i_stk,j_mat] = AS(sigma=sigma, r=r, q=q, So=So, k=k, T=T, b=b, delta=delta, lmbda=lmbda, nubar=nubar, eta=eta, nu_o=nu_o, rho=rho, mod=mod)
            ...
            figures[mod].update_layout(
                title=f"Heston IV", title_x=0.5, title_y=0.9,
                scene=dict(
